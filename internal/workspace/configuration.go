...
func validateTemplateConfiguration(configuration domain.TemplateConfiguration) error {
	if len(configuration.Command) > 32 || len(configuration.Environment) > 64 || len(configuration.Mounts) > 16 || len(configuration.Services) > 16 {
...
func resolveConfiguration(configuration domain.TemplateConfiguration, workspaceID, workspaceName string, allocations []domain.PortAllocation) (runtime.WorkspaceConfiguration, error) {
	...
	for _, value := range configuration.Environment {
		...
		result.Environment = append(result.Environment, runtime.EnvironmentVariable{Name: value.Name, Value: resolved, Sensitive: value.Sensitive})
	}
	for _, value := range configuration.Mounts {
