...
func (s *Store) CreateWorkspace(ctx context.Context, workspace domain.Workspace) error {
	_, err := s.db.ExecContext(ctx, `INSERT INTO workspaces
		(id, owner_user_id, template_id, template_revision, template_configuration_json, template_image_reference, template_image_digest, name, desired_state, observed_state, runtime_id, observed_error,
		...
		workspace.TemplateRevision, templateConfigurationJSON(workspace.TemplateConfiguration), workspace.TemplateImageReference, workspace.TemplateImageDigest, workspace.Name, workspace.DesiredState, workspace.ObservedState, workspace.RuntimeID, workspace.ObservedError,
...
const workspaceSelect = `SELECT id, owner_user_id, template_id, name, desired_state, observed_state, runtime_id,
	template_revision, template_configuration_json, template_image_reference, template_image_digest, observed_error, allocated_cpu_millis, allocated_memory_bytes, allocated_storage_bytes, created_at, updated_at,
...
func scanWorkspace(row scanner) (domain.Workspace, error) {
	...
	if err := row.Scan(&workspace.ID, &workspace.OwnerUserID, &workspace.TemplateID, &workspace.Name, &desiredState,
		&workspace.ObservedState, &workspace.RuntimeID, &workspace.TemplateRevision, &templateConfiguration, &workspace.TemplateImageReference, &workspace.TemplateImageDigest, &workspace.ObservedError, &workspace.AllocatedCPUMillis,
