...
| xterm.js | xterm.js 5.3.0, vendored locally, with a JetBrains Mono Nerd Font-first local font stack | Specialized terminal rendering should not be recreated. The local browser font stack preserves modern monospace text and Powerline glyphs without adding a font download or build step. MIT license, browser runtime only, no Node.js or npm requirement in COWS builds. |
